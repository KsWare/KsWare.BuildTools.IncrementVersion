...
using var log = new StreamWriter(new MemoryStream(), Encoding.UTF8);
// var log = Console.Out;
try {
	...
	for (int i = 0; i < args.Length; i++) {
		switch (args[i].ToLower()) {
			...
			case "-h":
			case "-help":
				return ShowHelpEndExit();
				;
			default:
				if (projectFile == null) projectFile = args[i];
				break;
		}
	}
	log.WriteLine($"CurDir:             {Environment.CurrentDirectory}");
	log.WriteLine("--- Parameter ---");
	log.WriteLine($"ProjectFile:        {projectFile}");
	log.WriteLine($"DesiredVersionFile: {versionFile}");
	log.WriteLine($"Version:            {desiredVersion}");
	log.WriteLine($"IsTestOnly:         {isTestOnly}");
	...
	*/
	...
	var versionProperties = Regex.Matches(content, @"<(?<name>[^>]*Version[^>]*)>\s*(?<content>[^<]*)\s*</\k<name>>")
		.Reverse().DistinctBy(m => m.Groups["name"]) // FIX for duplicate entries in project file
	...
	if (desiredVersion == null) {
		...
		if (versionFile != null) {
			...
			if (versionProperties.ContainsKey("VersionPrefix"))
				info = versionProperties["VersionPrefix"].MajorMinor().ToString(2);
			else if (versionProperties.ContainsKey("ApplicationDisplayVersion"))
				info = versionProperties["ApplicationDisplayVersion"].MajorMinor().ToString(2);
			else if (versionProperties.ContainsKey("Version"))
				info = versionProperties["VersionPrefix"].MajorMinor().ToString(2);
			else if (versionProperties.ContainsKey("AssemblyVersion"))
				info = versionProperties["VersionPrefix"].MajorMinor().ToString(2);
	...
	log.WriteLine($"Desired version:    {desiredVersion}{desiredSuffix}");
	...
	if (versionProperties.ContainsKey("ApplicationDisplayVersion")) {
		...
		if (versionProperties.ContainsKey("VersionPrefix"))
			current = versionProperties["VersionPrefix"].MajorMinorBuild();
		else if (versionProperties.ContainsKey("Version")) current = versionProperties["Version"].MajorMinorBuild();
		else if (versionProperties.ContainsKey("AssemblyVersion"))
			current = versionProperties["AssemblyVersion"].MajorMinorBuild();
	...
	if (!isTestOnly) {
		if (versionFile != null) {
			...
			w.ReplaceTag("PackageReleaseNotes", GetReleaseNotes(versionFile));
		}
	...
	using var f = File.CreateText("IncrementVersion.log");
	f.WriteLine(new StreamReader(log.BaseStream).ReadToEnd());
...
string GetReleaseNotes(string versionFile){
	using var infoFile = File.OpenText(versionFile);
	var lines = new List<string>();
	var line = infoFile.ReadLine();
	while (line!=null) {
		line = infoFile.ReadLine();
		if (line == null) break;
		if (Regex.IsMatch(line, @"^# \d+\.\d+")) break;
		lines.Add(line);
	}
...
class Writer {
	...
	public void ReplaceTag(string name, string value) {
